    ...
    switch($action) {
      case 'install':
        try {
          ...
          $this->db->ExecuteQuery(self::SQL('insert into user'), array('root', 'The Administrator', 'Daniel@djmacz.se', $password['algorithm'], $password['salt'], $password['password']));
          $idRootUser = $this->db->LastInsertId();
          $password = $this->CreatePassword('doe');
          $this->db->ExecuteQuery(self::SQL('insert into user'), array('doe', 'John/Jane Doe', 'Daniel@djmacz.se', $password['algorithm'], $password['salt'], $password['password']));
...
*/
  public function CreatePassword($plain, $algorithm=null) {
    $password = array(
      'algorithm'=>($algorithm ? $algoritm : CSkola::Instance()->config['hashing_algorithm']),
      'salt'=>null
    );
